...
mask <- as.matrix(read.csv(list.files(pattern="mask0.")))
final_data <- as.data.frame(read.csv(list.files(pattern="sim0.data.")))
...
transformed parameters {
  ...
  for (i in 1:N) {
    lambda[i] = lambda0 * exp(alpha11 * x1[i] + alpha12 * x2[i] + b * b_i[i]);
  }
...
model {
  ...
  for (i in 1:N) {
    for (t in 1:T) {
      ...
      if (mask[i, t] > 0) {
        y[i, t] ~ normal(alpha00 + x1[i] * alpha01 + x2[i] * alpha02 + backward_time * alpha03 + treatment[i] * alpha04 + b_i[i], sigma_e);
      }
    }
  }
...
init_fn <- function() {
  list(alpha00 = 10, alpha01 = 5, alpha02 = 0.3, alpha03 = 2, alpha04 = 10, alpha11 = 1, alpha12 = 0.05, b = 0.03, lambda0 = 0.05, gamma = 1.2, sigma_b = 8, sigma_e = 3)
}

# Compile and sample from the Stan model
fit <- sampling(stan_model, data = stan_data, init = init_fn, iter = 4000, warmup = 2000, chains = 2, control = list(adapt_delta = 0.99, max_treedepth = 12), cores=2)

result <- summary(fit)
fit_df <- as.data.frame(result$summary)
text <- list.files(pattern="sim0.data.")
num <- unlist(lapply(strsplit(text,'.',fixed=TRUE),function(x) x[[3]]))
write.csv(fit_df, paste0("mod0.result.",num,".csv"))


pdf(file = paste0("mod0.traceplot.",num,".pdf"),   # The directory you want to save the file in
