...
for (sim in 1:num_simulations) {
  ...
  linear <- alpha11 * x1 + alpha12 * x2  + b * bi 
  ...
  for (i in 1:n) {
    for (ind in seq_along(time_points)) {
      ...
      if (backward_time > 0) {
        measurement <- alpha00 + x1[i] * alpha01 + x2[i] * alpha02 + backward_time * alpha03 + treatment[i] * alpha04 + bi[i] + epsiloni[i, ind]
      } else {
        measurement <- NA
      }
  ...
  filename <- paste0("mask0.", sim-1, ".csv")
  write.csv(mask_df, file = filename, row.names = FALSE)
  
  filename <- paste0("sim0.data.", sim-1, ".csv")
